...
현재는 랜덤포레스트(Random Forest) 기반의 패킷 분류 및 탐지 방식을 사용하고 있으나, 앞으로는 강화학습(DQN, PPO 등) 알고리즘과 유기적으로 연동하여 하이브리드 보안 에이전트로 발전시킬 계획입니다.

- 랜덤포레스트의 예측 결과(확률, feature 등)를 강화학습(RL) 에이전트의 상태(state)로 활용하거나, RL의 행동(action) 결정에 반영할 예정입니다.
- RL 알고리즘(DQN, PPO 등)을 도입하여, 실시간 네트워크 환경 변화에 적응하고, 정책 최적화 및 자동화된 보안 대응이 가능하도록 시스템을 확장할 계획입니다.
- 두 알고리즘의 장점을 결합하여, 더 높은 탐지율과 적응성을 갖춘 네트워크 보안 에이전트로 발전시킬 예정입니다.
