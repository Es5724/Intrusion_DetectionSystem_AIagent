...
1. [📌 프로젝트 개요](#1-프로젝트-개요)
2. [👥 팀원 정보](#2-팀원-정보)
3. [🏗️ 프로젝트 구조](#️3-프로젝트-구조)
4. [🛠️ 사용된 모듈 및 라이브러리](#️4-사용된-모듈-및-라이브러리)
...
- **pandas**
     - 데이터 조작 및 분석을 위한 Python 라이브러리로, DataFrame과 Series 자료구조 제공
         - 네트워크 패킷 데이터 로드, CSV 파일 처리, 결측치 처리, 범주형 데이터 인코딩 및 데이터 전처리 파이프라인 구축에 활용
- **numpy**: 수치 계산을 위한 라이브러리
     - 다차원 배열 객체와 수학 함수를 제공하는 과학적 컴퓨팅 라이브러리
         - 패킷 데이터 벡터화, 수치형 특성 정규화, 머신러닝 모델 입력 데이터 변환 및 행렬 연산에 사용
